...
exports.syncMyPuzzlePower = exports.adminRecalcStagePuzzlePower = exports.adminRebalanceFakeUsers = exports.adminRecalcPuzzlePower = exports.adminMigrateLegacyCoins = exports.refundPortOnePayment = exports.adminGetPayments = exports.adminSaveUserData = exports.adminGetUserData = exports.verifyPortOnePayment = void 0;
...
exports.adminMigrateLegacyCoins = (0, https_1.onCall)(async (request) => {
    assertAdmin(request);
...
exports.adminRecalcPuzzlePower = (0, https_1.onCall)({ timeoutSeconds: 540, memory: "512MiB" }, async (request) => {
    ...
    try {
        ...
        for (const snap of challengeDocs.docs) {
            ...
            const claimed = data.claimedIds ?? [];
            const syncedPP = data.mainDocSyncedPP ?? 0;
            ...
            if (delta === 0 && data.puzzlePower === newPP)
                continue;
            changed++;
            totalDelta += delta;
        ...
        if (!dryRun) {
            ...
            for (let i = 0; i < updates.length; i += BATCH_SIZE) {
                ...
                for (const u of updates.slice(i, i + BATCH_SIZE)) {
                    ...
                    if (u.delta !== 0) {
                        batch.set(db.collection("users").doc(u.uid), { puzzlePower: firestore_1.FieldValue.increment(u.delta) }, { merge: true });
                    }
                }
                await batch.commit();
            }
...
const STAGE_PROGRESS_SOURCES = [
    {
        ...
        cleared: (d) => Math.max(0, Number(d.beginnerProgress) || 0) +
 ...
 */
function sudokuRegularCleared(progressData, userData) {
    const candidates = [
 ...
 */
async function recomputeAllPuzzlePower(userDocs) {
    const stagePP = new Map();
    const challengePP = new Map();
    /** 서브컬렉션에 들어 있는 스도쿠 일반 진행도 (옛 위치와 비교해 큰 쪽을 쓴다) */
    const sudokuProgressDocs = new Map();
    ...
    STAGE_PROGRESS_SOURCES.forEach((source, i) => {
        for (const doc of snaps[i].docs) {
            ...
            if (!uid || uid.startsWith("fake_user_"))
                continue;
            const data = doc.data() ?? {};
            if (source.collection === "sudokuProgress")
                sudokuProgressDocs.set(uid, data);
...
exports.adminRecalcStagePuzzlePower = (0, https_1.onCall)({ timeoutSeconds: 540, memory: "512MiB" }, async (request) => {
    assertAdmin(request);
    const dryRun = request.data?.dryRun !== false;
    try {
        ...
        const userSnap = await db.collection("users")
            ...
            .get();
        const userDocs = new Map(userSnap.docs.map((d) => [d.id, d.data()]));
        const current = new Map(userSnap.docs.map((d) => [d.id, d.data().puzzlePower ?? 0]));
        ...
        for (const u of users.values()) {
            if (!current.has(u.uid))
                continue; // 프로필 문서가 없는 유령 진행도는 건너뛴다
        ...
        if (!dryRun) {
            ...
            for (let i = 0; i < planned.length; i += BATCH_SIZE) {
                ...
                for (const u of planned.slice(i, i + BATCH_SIZE)) {
                    batch.set(db.collection("users").doc(u.uid), { puzzlePower: u.newPP }, { merge: true });
                    // 클라이언트는 (도전과제 PP - mainDocSyncedPP) 만큼을 다시 더한다.
                    // 여기서 맞춰 두지 않으면 다음 로그인 때 중복으로 더해진다.
                    batch.set(db.collection("users").doc(u.uid).collection("challenges").doc("data"), { mainDocSyncedPP: u.challengePP }, { merge: true });
                }
                await batch.commit();
            }
        }
        ...
        throw new https_1.HttpsError("internal", `스테이지 퍼즐력 재계산 실패: ${msg}`);
    }
});
/**
 * 본인 퍼즐력 재동기화.
 ...
 */
exports.syncMyPuzzlePower = (0, https_1.onCall)(async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "로그인이 필요합니다.");
    if (uid.startsWith("fake_user_"))
    ...
    STAGE_PROGRESS_SOURCES.forEach((source, i) => {
        ...
        if (snap.exists)
            stagePP += source.cleared(snap.data() ?? {});
    });
    const sudokuSnap = progressSnaps[STAGE_PROGRESS_SOURCES.findIndex((s) => s.collection === "sudokuProgress")];
    stagePP += sudokuRegularCleared(sudokuSnap?.exists ? sudokuSnap.data() : undefined, userSnap.data());
    const claimed = challengeSnap.exists ? (challengeSnap.data()?.claimedIds ?? []) : [];
    const challengePP = claimed.filter((id) => VALID_CHALLENGE_IDS.has(id)).length * PUZZLE_POWER_PER_CHALLENGE;
    const newPP = stagePP + challengePP;
    const before = userSnap.data()?.puzzlePower ?? 0;
    if (before === newPP)
        return { changed: false, puzzlePower: newPP };
